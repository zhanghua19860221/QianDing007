...
- (void)ccSubmitBtnBlick{
  ...
  if ([com_cardedField.text isEqualToString:@""]||![shareDelegate isLenghtCard:com_cardedField.text]) {
    ...
    [[shareDelegate shareAFHTTPSessionManager] POST:[shareDelegate stringBuilder:SAVEINFOUSER_URL] parameters:companyDic constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
...
//    //创建请求菊花进度条
    ...
    [[shareDelegate shareAFHTTPSessionManager] POST:[shareDelegate stringBuilder:GETINFOUSER_URL] parameters:levelDic progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)createOneView{
    ...
    com_companyNameField.placeholder = @"与营业执照一致（必填）";
    ...
    com_creditField.placeholder = @"（必填）";
    ...
    com_detailAddressField.placeholder = @"（必填）";
    ...
    com_userNameField.placeholder = @"法人（必填）";
    ...
    com_cardedField.placeholder = @"与填写姓名保持一致（必填）";
    ...
    com_telePhoneField.placeholder = @"（必填）";
    ...
    com_accountField.placeholder = @"（必填）";
    ...
    com_branceTeleField.placeholder = @"（必填）";
